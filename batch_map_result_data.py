"""
Batch mapping script for multiple result JSON files.
Processes all JSON files in a directory and creates mapped result data and documents.
"""
...
def process_single_file(input_file: str, output_dir: str = None) -> tuple:
    """
    Process a single result JSON file and create mapped outputs.
    ...
    Returns:
        Tuple of (success: bool, mapped_results_count: int, mapped_documents_count: int, error_message: str)
    """
    try:
        ...
        print(f"  Processing result data mapping...")
        mapped_results = process_aoc_file(input_file, output_path)
        results_count = len(mapped_results) if mapped_results else 0
        # Clear from memory after getting count
        del mapped_results
        
        # Process result documents mapping
        # This function writes the file immediately and returns the data
        print(f"  Processing result documents mapping...")
        mapped_documents = map_result_documents(input_file, documents_output_path)
        documents_count = len(mapped_documents) if mapped_documents else 0
        # Clear from memory after getting count
        del mapped_documents
        
        return True, results_count, documents_count, None
...
def batch_map_results(directory: str, output_dir: str = None):
    """
    Process all JSON files in a directory and create mapped result data and documents.
    ...
    """
    print("=" * 60)
    print("BATCH MAPPING RESULT DATA")
    print("=" * 60)
    ...
    for index, input_file in enumerate(json_files, 1):
        ...
        success, results_count, documents_count, error_msg = process_single_file(
        ...
        if success:
            ...
            print(f"  ✓ Successfully mapped: {results_count} results, {documents_count} documents")
    ...
    print()
    print(f"Total results mapped: {total_results}")
...
if __name__ == "__main__":
    ...
    batch_map_results(input_directory, output_directory if output_directory else None)
